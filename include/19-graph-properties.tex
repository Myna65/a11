...
\begin{proof}
  ...
  \paragraph{}
  Suppose now that $i$ and $k$ are not consecutive, an alternating square. \footnote{to do}
  Thus $i$ and $k$ must be consecutive and $k = i+1$ (up to duality). But by isomorphim the same applies to $j$ and $l$ and thus $l = j+1$.
      ...
      \begin{tikzpicture}[scale=.8]
        ...
        \begin{scope}[every node/.style={fill=white, transform shape}]
          ...
          \begin{scope}[every edge/.style={draw}]
            ...
            \path (4)  edge node {$i$} (5);
            \path (7)  edge node {$i$} (8);
            \path (1)  edge node {$j+1$} (4);
            \path (2)  edge node {$j+1$} (5);
            \path (3)  edge node {$j+1$} (6);
            \path (4)  edge node {$j$} (7);
            \path (5)  edge node {$j$} (8);
          \end{scope}
        \end{scope}
