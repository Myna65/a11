...
\begin{proof}
  ...
  \paragraph{}
  In this case there are 14 edges to link 11 vertices. Thus there are 4 "joker" edges. But two of them have already been used to build the alternating square and the double edge.

  \paragraph{}
  We place the $\rho_0$ edges. We know by the proof of the previous lemma that one edge must be adjacent to the single $\rho_1$ edge.
  ...
  \paragraph{}
  It is clear here that $\rho_0 = (\rho_2 \rho_3)^7$. Thus the last $\rho_2$ edge must triple to existing double edge. Therefore the last $\rho_2$ edge must be $(b,d)$.
