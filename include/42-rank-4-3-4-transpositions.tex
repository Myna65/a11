...
\begin{proof}
  ...
  \paragraph{}
  There are only 4 $\rho_2$ waiting to be placed. Three of them must link the components. The last one must double someting. The only two possibilities: triple the double edge $(\rho_0, \rho_3)$ or double the other $\rho_0$.
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.8]
        ...
        \begin{scope}[every node/.style={fill=white}]
          ...
          \begin{scope}[every edge/.style={draw}]
            \path (1)  edge[bend left=45] node {$0$} (3);
            \path (7)  edge node {$0$} (11);
            \path (1)  edge node {$1$} (2);
            \path (3)  edge node {$1$} (4);
            \path (5)  edge[bend left=30] node {$1$} (6);
            ...
            \path (1)  edge[bend right=45] node {$3$} (3);
            \path (2)  edge node {$3$} (4);
            \path (5)  edge[bend right=30] node {$3$} (6);
            \path (9)  edge node {$3$} (10);
          \end{scope}
