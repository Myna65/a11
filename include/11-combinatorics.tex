...
\paragraph{}
In the figure, there is a link between $a$ and $b$ so $a \le b$. For the same reason $b \le d$. So $a \le d$ by transitivity, even if there is no line between those points.
...
\paragraph{}
On the Hasse diagram, it means that, if all elements with the same rank are placed at the same level, an element is only connected to elements placed the level just above or just under itself.
...
\begin{figure}[H]
  \begin{center}
    \begin{tikzpicture}
      ...
      \begin{scope}[every node/.style={circle,draw,fill=white, transform shape}]
        \node (f)  at (1,6)  {f};
        \node (e)  at (2,4)  {e};
        \node (d)  at (0,4)  {d};
        \node (c)  at (2,2)  {c};
