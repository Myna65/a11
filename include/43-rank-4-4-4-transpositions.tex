...
\begin{proof}
  ...
  \paragraph{}
  If the sequence is extended, the possible adjacent squares are $[\rho_0,\rho_3]$, $[\rho_2, \rho_3]$ or $[\rho_0, \rho_2]$\footnote{Not finished}.
  ...
  \paragraph{}
  Now, there are four points remaining. We can build and alternating square. This square must be $[\rho_1, \rho_3]$ but then there are two $\rho_0$ edge and no $\rho_1$. No new alternating square can be built because there not enough points. Thus that is impossible and no new square can be built.

  \paragraph{}
  No $\rho_0$ edge can be placed inside existing square and there are two $\rho_0$ edges remaining for one $\rho_1$ and thus by Proposition~\ref{rho0atEnd} the graph will never be completed.
  ...
  \paragraph{}
  Thus none of the two alternating square can be extended. Thus they must be linked by a single edge.
  ...
  \paragraph{}
  There are three possibilities to link both component a chain $\rho_0, \rho_1$, a chain $\rho_2, \rho_1$ or a chain $\rho_3, \rho_1$. The last case is the dual of the first case.
  ...
  \paragraph{}
  The only solution is $A_{11}$. But if $\Gamma_{\rho_0, \rho_1, \rho_2} = \Gamma = A_{11}$, then $\Gamma$ cannot satisfies the intersection property by~\ref{not-monodromy-intersection}.
  ...
  \paragraph{}
  There are 16 edges to link 11 vertices, thus we have 6 "joker" edges. But we built three alternating square and three double edges and thus all of them have been used. All new edges must link two different components.
