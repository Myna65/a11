...
\begin{proof}
  ...
  \paragraph{}
  There is the same problem in the horizontal direction. Thus two other alternating square must be built an $b, e, h$ and $a, d, g$ because $\rho_{i+1}$ and $\rho_{i+3}$ does not commute.
...
\begin{proposition}
  ...
  In a polytope of rank 5 or less and on 11 points or less, every non-monotone linear sequence of alternating squares contains one of the following two patterns or a simple cycle with three squares.
...
\begin{proof}
  ...
  \paragraph{}
  If $i$ and $k$ are not consecutive then a $\rho_i$ edge must be placed between $e$ and $f$. However, by Proposition~\ref{square-connection}, an additionnal square must be built. In order to end the sequence at one moment the horizontal component of the last square must be adjacent to $i$ or $l$.
  ...
  \paragraph{}
  If there are ten points: there are two possible patterns:
  ...
  \paragraph{}
  If there are more than eight points, then the rank of polytope will be greater than five or the pattern constructed will include the pattern of Figure~\ref{linear-1}.
