...
\paragraph{}
Note that the word \textit{edge} without adjectives conserves its original meaning of the graph theory. We only define a special type of edge here.
...
\begin{proposition}
  ...
  If a sequence of alternating squares is not linear, then it contains a cycle of 4 squares or it contains the following pattern.
...
\begin{proof}
  So we start with four indices $i, j, k, l$, all differents and two squares that shares exactly one vertex.
  ...
  \paragraph{}
  Suppose now that $j$ and $k$ are not consecutive, then another alternating square must be built on the points $a,d,f$. This square must use a new point. The graph is now:

  \paragraph{}
  But there is a cycle of 4 squares is this graph. Thus $j$ and $k$ must be consecutive, thus $j = k+1$ up to duality.
  ...
  \paragraph{}
  Suppose now that $i$ and $k$ are not consecutive, then two alternating squares must be built on $c,d,e$ and $f,g,h$. None of those squares can use existing points. The graph is the folowing:
  ...
  \paragraph{}
  Now if $i$ and $l$ are not consecutive then a $\rho_l$ edge must be added between the points $i$ and $j$. This edge form a loop of four alternating squares. Therefore $i$ and $l$ must commute and $l = i+1$. Moreover the same applies to $k$ and $i+1$, thus $k = i + 2$.
  ...
  \paragraph{}
  But there is the same problem in the horizontal direction. Thus two other alternating square must be built an $b, e, h$ and $a, d, g$ because $\rho_{i+1}$ and $\rho_{i+3}$ does not commute.
  ...
  \paragraph{}
  But the a $i$ edge must be placed between $k$ and $l$ because $\rho_i$ and $\rho_{i+3}$ does not commute. Thus there is a cycle of four squares.
...
\begin{proposition}
  ...
  Every non-monotone linear sequence of alternating squares contains the following pattern or a cycle.
