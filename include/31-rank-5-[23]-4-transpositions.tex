...
\paragraph{}
Now we add the 2-transposition $\rho_4$ keeping in mind that $\rho_1$ and $\rho_4$ must commute. Lemma~\ref{patterns-adding} applies and there are only three possibilities to place the two $\rho_4$ edges.
...
\paragraph{}
Now we have found a set of possible permutation representation group. But in all those four graphs, one generator can be written as a product of other generators. To demonstrate this, we only choose some edges of the graphs.

\paragraph{}
If the $\rho_0 $ and $\rho_3$ edges are removed, all the graphs become to following one:
...
\paragraph{}
The two $\rho_0$ edges cannot be connected to any vertex of the left component, thus $a, b, c, d, e, f$ by Proposition~\ref{square-connection}. The point $k$ cannot be connected by two $\rho_0$ edge thus there is a single possibility to place $\rho_0$ edges at isomorphism: link $(h, i)$ and $(j, k)$.
...
\paragraph{}
The graph is now connected, we begin the last part of the method~\ref{method}: place all remaining edges to satisfy parity. Thus at least one $\rho_2$ edge must still be placed. The only available edges are $f,h,i,j$ and $k$. But $f$ cannot be connected to any other point by a $\rho_0$ edge because it must form an alternating square but this square does not satisfies Proposition~\ref{adjacent-squares}. Thus an edge $(h,i)$ or an edge $(j,k)$ must be built. That gives us two cases.
...
\paragraph{}
By Proposition~\ref{chain-consecutive} and since all $\rho_4$ edge have been used and since no $\rho_2$ can be connected to the alternating square, a $\rho_2$ edge must be used to connect the two components together.
...
\paragraph{}
In this situation the graph must be extended to the left or to the right. But the situation is isomorphic. The vertical edge of the added must $\rho_1$ because $\rho_4$ is a 2-transposition. The horizontal edge must be $\rho_3$ by Propositions~\ref{square-connection} and~\ref{continue-double-edge}.
...
\paragraph{}
There are only 4 edges available to connect 5 components. Thus every edge must link two different components. Each square must be link by a $\rho_2$ edge by Proposition~\ref{square-connection}.
