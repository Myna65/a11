...
\paragraph{}
The graph is now connected, we begin the last part of the method~\ref{method}: place some remaining edge to satisfies parity. Thus at least one $\rho_2$ edge must still be placed. The only available edges are $f,h,i,j$ and $k$. But $f$ cannot be connected to any other point by a $\rho_0$ edge because it must form an alternating square but this square does not satisfies Proposition~\ref{adjacent-squares}. Thus an edge $(h,i)$ or an edge $(j,k)$ must be built. That gave us two cases.
