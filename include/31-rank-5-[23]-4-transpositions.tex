...
\paragraph{}
The graph is connected, thus the group is transitive on 11 points. Moreover, this group is 2-transitive. To prove this we use the Proposition~\ref{practical-transitivity}: we fix the point $k$ and try to keep $k$ fixed while finding permutation between $j$ and all non-fixed points. This is trivial because the point $k$ does not move if no $\rho_0$ edges are placed. All the other points are in the same orbit of $\Gamma_{\rho_1, \rho_2, \rho_3}$ and thus $j$ can be permuted with any non-fixed point without using a $\rho_0$ edge.
