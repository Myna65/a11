...
\begin{proof}
  In Propositions~\ref{rotation-patterns} and~\ref{linear-pattern}, we found two special cycles. But none of those cycles can be connected by a simple edge.

  \paragraph{}
  So the only cycles that are left are linear monotone cycle or cycles that contains either the pattern~\ref{rotation-pattern} or one of the patterns from~\ref{linear-pattern}. But for both of those patterns, if a simple edge is attached
\end{proof}
