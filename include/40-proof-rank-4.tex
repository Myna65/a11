...
\begin{proof}
  ...
  \paragraph{}
  But now we must insert two $\rho_2$ edges or two $\rho_3$ edges which is impossible. Thus the edge must be $\rho_2$.
  ...
  \paragraph{}
  If the chain is extended it must be extended by a $\rho_1$ because if we use a $\rho_3$ then $\rho_2$ and $\rho_3$ would be 4-transposition. Thus two double edges must still be placed which is impossible. If the chain is extended, the next single edge must be a $\rho_0$ edge by Property~\ref{rho0atEnd}. There is a $\rho_2$ edge remaining and a fixed point but they cannot make a connected graph. Thus the chain cannot be extended.

  \paragraph{}
  Another $\rho_1$ edge must connect the alternating square to a fixed point. This edge must also be followed by a $\rho_0$.
