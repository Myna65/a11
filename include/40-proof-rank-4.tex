...
\begin{proof}
  ...
  \paragraph{}
  If the $\rho_0$ edge are single edges or are part of double edges they needs one $\rho_1$ edge each to be connected to the graph\footnote{More precise}. But that is impossible because there are only two of them. Thus the $\rho_0$ edges must be part of an alternating square.

  \paragraph{}
  The distance between the indices of the alternating square must be 2\footnote{to do}. Thus there are two alternating square $[\rho_0, \rho_2]$ and $\rho_2$ is a 4-transposition. Each square must be connected to the rest of the graph by a $\rho_1$ edge. Now all $\rho_0, \rho_1$ and $\rho_2$ edge have been used but the graph is not connected and there are only $\rho_3$ edges that cannot connected two distincit components.
